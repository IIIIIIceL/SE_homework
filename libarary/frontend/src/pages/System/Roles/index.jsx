...
export default function RolesPage() {
  ...
  let listContent;
  if (loading) {
    listContent = <div className={styles.loading}>正在加载角色...</div>;
  } else if (roles.length === 0) {
    listContent = <div className={styles.empty}>暂无角色</div>;
  } else {
    listContent = (
      <table className={styles.table}>
        <thead>
          <tr><th>名称</th><th>描述</th><th>用户数</th><th>操作</th></tr>
        </thead>
        <tbody>
          {roles.map((role) => (
            <tr key={role.id}>
              ...
              <td className={styles.actions}>
                <button className={styles.linkBtn} onClick={() => setEditingRole(role)}>编辑</button>
                <button className={`${styles.linkBtn} ${styles.danger}`} onClick={() => handleDelete(role.id)}>删除</button>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <div className={styles.split}>
        <section className={styles.panel}>
          ...
          <form className={styles.filterBar} onSubmit={(event) => { event.preventDefault(); setPage(1); setAppliedKeyword(keyword); }}>
            <input className={styles.input} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="搜索角色名称或描述" />
        ...
        <section className={styles.panel}>
          <div className={styles.panelHeader}><h3 className={styles.sectionTitle}>{editingRole?.id ? '编辑角色' : '创建角色'}</h3></div>
          <RoleForm mode={editingRole?.id ? 'edit' : 'create'} initialData={editingRole} onSubmit={handleSubmit} onCancel={() => setEditingRole(null)} submitting={submitting} />
        </section>
      </div>
    </div>
  );
