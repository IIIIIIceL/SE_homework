...
export default function BorrowDetail() {
  ...
  const fetchRecord = useCallback(async () => {
    setLoading(true);
    try {
      const data = await borrowService.getBorrowDetail(id);
      setRecord(data);
    } catch (err) {
      setError(err.response?.data?.message || '获取借阅详情失败');
    } finally {
      setLoading(false);
    }
  }, [id]);
  ...
  const handleRenew = async () => {
    ...
    if (!dueDate) return;
    try {
      await borrowService.renewBook(id, { newDueDate: dueDate });
      alert('续借成功');
      fetchRecord();
    } catch (err) {
    ...
    <div className={styles.page}>
      <div className={styles.header}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {canOperate && (
            <>
              <button className={styles.btn} onClick={handleReturn}>归还</button>
              <button className={styles.btn} onClick={handleRenew}>续借</button>
            </>
          )}
        </div>
      </div>
      ...
      <div className={styles.detailCard}>
        <dl className={styles.detailGrid}>
          <dt>借阅号</dt><dd>{record.borrowNo}</dd>
          <dt>读者</dt><dd>{record.reader?.name || '-'} ({record.reader?.readerNo || '-'})</dd>
          <dt>书籍</dt><dd>{record.book?.title || '-'}（{record.book?.author || '-'}）</dd>
          ...
          <dt>操作人</dt><dd>{record.borrowOperator?.fullName || '-'}</dd>
        ...
        <div className={styles.timeline}>
          <div className={`${styles.timelineItem} ${styles.done}`}>
            <div className={styles.label}>借出</div>
            <div className={styles.time}>{record.borrowDate ? new Date(record.borrowDate).toLocaleString() : '-'}</div>
          </div>
          {record.renewCount > 0 && (
            <div className={styles.timelineItem}>
              <div className={styles.label}>续借（{record.renewCount} 次）</div>
            </div>
          )}
          {record.returnDate ? (
            <div className={`${styles.timelineItem} ${styles.done}`}>
              <div className={styles.label}>归还</div>
              <div className={styles.time}>{new Date(record.returnDate).toLocaleString()}</div>
            </div>
          ) : (
            <div className={styles.timelineItem}>
              <div className={styles.label}>待归还</div>
              <div className={styles.time}>应还：{record.dueDate ? new Date(record.dueDate).toLocaleDateString() : '-'}</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ marginTop: 16 }}>
        <Link to="/borrows" style={{ color: '#1890ff' }}>← 返回列表</Link>
      </div>
    </div>
  );
}
